...
describe("TextShimmer", () => {
  test("stops the sweep under reduced motion", () => {
    ...
    const shimmer = container.querySelector(".agentui-text-shimmer");
    ...
    expect(style?.textContent).toContain(".agentui-text-shimmer");
  ...
  test("carries the marker class on every shimmering element", () => {
    ...
    const shimmering = container.querySelectorAll('[style*="agentui-text-shimmer"]');

    expect(shimmering.length).toBeGreaterThan(0);
    for (const element of shimmering) {
      expect(element.className).toContain("agentui-text-shimmer");
    }
  });
...
describe("TEXT_SHIMMER_CLASS_NAME", () => {
  test("carries the marker the reduced-motion rule selects", () => {
    expect(TEXT_SHIMMER_CLASS_NAME).toContain("agentui-text-shimmer");
  });
});
